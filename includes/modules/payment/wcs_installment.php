...
class wcs_installment extends WirecardCheckoutSeamlessPayment
{
    ...
    function selection()
    {
        ...
        if ($this->getConfigParam('PROVIDER') == 'payolution' && $this->getConfigParam('PAYOLUTION_TERMS') == 'True') {
            ...
            $content['fields'][] = array(
                'title' => sprintf('<input id="%s" type="checkbox" name="consent" autocomplete="off" class="wcs_installment consent"/>',
                    $fieldId),
...
HTML;
    ...
    {
        if (isset($_POST['wcs_installment_birthday'])) {
            $_SESSION['wcs_birthday'] = $_POST['wcs_installment_birthday'];
        }
    }
    ...
    protected function _configuration()
    {
        ...
        $config['PROVIDER'] = array(
            ...
            'set_function' => "wcs_installment_cfg_pull_down_provider( "
        );
 ...
 * @return string
 */
function wcs_installment_cfg_pull_down_provider($provider, $key = '')
{
